...
public class StickHeaderDecoration extends RecyclerView.ItemDecoration {
    ...
    public StickHeaderDecoration(Context context) {
        ...
        mTextPaint.setTextSize(ResourceUtils.dp2px(context,12));
        mTextPaint.setColor(0xff454545);
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        if (parent.getAdapter() instanceof MStickHeaderAdapter) {
            MStickHeaderAdapter adapter = (MStickHeaderAdapter) parent.getAdapter();
            int count = parent.getChildCount();//获取可见范围内Item的总数
            for (int i = 0; i < count; i++) {
                View view = parent.getChildAt(i);
                int position = parent.getChildLayoutPosition(view);
                boolean isHeader = adapter.isItemHeader(position);
                int left = ResourceUtils.dip2px2(MyApplication.getApp(),16);
